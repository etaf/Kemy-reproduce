...
if topo == 'Dumbbell':
    ...
    conffile = "./kemyconf/test-square.tcl"
    MIN_RTT = 150
else:
...
if  os.path.exists(results_dir):
    ...
    childs = []
    for nsrc in nsrcs:
        childs.append(subprocess.Popen('python runkemy2.py -c %s -d %s -q DropTail -p %s -n %d -a %d --topo=%s' % \
                (conffile,results_dir,src_proto,nsrc,iterations,topo),shell=True))
        if not topo == 'adhoc':
            childs.append(subprocess.Popen('python runkemy2.py -c %s -d %s -q Blue -p %s -n %d -a %d --topo=%s' % \
                (conffile,results_dir,src_proto,nsrc,iterations,topo),shell=True))
        childs.append(subprocess.Popen('python runkemy2.py -c %s -d %s -q RED -p %s -n %d -a %d --topo=%s' % \
                (conffile,results_dir,src_proto,nsrc,iterations,topo),shell=True))

        childs.append(subprocess.Popen('python runkemy2.py -c %s -d %s -q CoDel -p %s -n %d -a %d --topo=%s' % \
                (conffile,results_dir,src_proto,nsrc,iterations,topo),shell=True))
        os.environ['WHISKERS'] = config.whiskers
        childs.append(subprocess.Popen('python runkemy2.py -c %s -d %s -q KEMY -p %s -n %d -a %d --topo=%s' % \
                (conffile,results_dir,src_proto,nsrc,iterations,topo),shell=True))
