...
if  os.path.exists(results_dir):
    ...
    childs = []
    for nsrc in nsrcs:
        childs.append(subprocess.Popen('python runkemy4nsrc.py -c %s -d %s -q DropTail  -a %d --topo=%s -s %d' % \
                (conffile,results_dir,iterations,topo,nsrc),shell=True))
        if not topo == 'adhoc':
            childs.append(subprocess.Popen('python runkemy4nsrc.py -c %s -d %s -q Blue  -a %d --topo=%s -s %d' % \
                (conffile,results_dir,iterations,topo,nsrc),shell=True))
        childs.append(subprocess.Popen('python runkemy4nsrc.py -c %s -d %s -q RED  -a %d --topo=%s -s %d' % \
                (conffile,results_dir,iterations,topo,nsrc),shell=True))

        childs.append(subprocess.Popen('python runkemy4nsrc.py -c %s -d %s -q CoDel  %d --topo=%s -s %d' % \
                (conffile,results_dir,iterations,topo,nsrc),shell=True))
        os.environ['WHISKERS'] = config.whiskers
        childs.append(subprocess.Popen('python runkemy4nsrc.py -c %s -d %s -q KEMY  %d --topo=%s -s %d' % \
                (conffile,results_dir,iterations,topo,nsrc),shell=True))
...
############ make graph #####################
pre_graph_name = "graph-compare-nsrc"
os.chdir("./graphing-scripts")
subprocess.call("./graphmaker-nsrc %s %s %f" % (results_dir,pre_graph_name,MIN_RTT), shell=True)
os.chdir(cwd)
